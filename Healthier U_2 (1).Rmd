...
check_packages(c("XML","RCurl","reshape","plyr","dplyr","broom","ggplot2"))   # Checks to see if required packages are already installed.
```
...
```{r}
#Use scraped data to calculate regression coefficients
library(plyr)
library(dplyr)
library(reshape)
library(broom)
...
###Note: This training regimen assumes client's diet stays consistent.
```{r}
weight <- 130
target.weight <- 120
